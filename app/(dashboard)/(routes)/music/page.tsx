      ...
      <div className="px-4 lg:px-8">
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="rounded-lg border w-full p-4 px-3 md:px-6 focus-within:shadow-sm grid grid-cols-12 gap-2">
                    <FormField name="prompt" render={({field})=>(<FormItem className="col-span-12 lg:col-span-10">
                        <FormControl className="m-0 p-0">
                            <Input className="border-0 outline-none focus-visible:ring-0 focus-visible:ring-transparent" disabled={isLoading} placeholder="generate a minute long music of piano" {...field} />
        ...
        <div className="space-y-4 mt-4">
          {isLoading && (
            ...
            {!music && !isLoading && (
            <div>
              <Empty label='No Music requested yet.' />
            </div>
            )}
            {music && (
